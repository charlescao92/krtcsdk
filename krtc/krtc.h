...
public:
    ...
    virtual void OnPureVideoFrame(int width, int height,
...
public:
    static void Init(CONTROL_TYPE type, KRTCEngineObserver* observer);
    ...
    static IMediaHandler* CreatePreview(int hwnd);
    static IMediaHandler* CreatePusher(const char* server_addr, const char* push_channel = "livestream");
    static IMediaHandler* CreatePuller(const char* server_addr, const char* pull_channel = "livestream", int hwnd = 0);
};
